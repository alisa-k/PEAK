...
public class HungarianAlgorithm {
	...
	private static int findLargest		//Finds the largest element in a positive array.
	(int[][] array)
	...
	private static int[][] copyOf	(int[][] costs)		//Copies all elements of an array to a new array.
		
	{
		int[][] copy = new int[costs.length][costs[0].length];
	...
	//public static int[][] hgAlgorithm (int[][] costs, String sumType)
	//public static int hgAlgorithm (int[][] costs, String sumType)
	public static int[] hgAlgorithm (int[][] costs, String sumType, String f) throws IOException
	{
		int[][] cost = copyOf(costs);	//Create the cost matrix
		...
		int[][] assignment = new int[costs.length][2];	//Create the returned array.
		...
		while((str = br.readLine()) != null) {
			...
			String tripsen = splited[1] + "\n" + splited[2];
			h.put(key, tripsen);
		}
		br.close();
		
		f = f.replace("TripleInfo.txt", "TripleAssignments.txt");
		File tripleAss = new File(f);
		PrintStream tripleAssPrintStream = new PrintStream(tripleAss);
		System.setOut(tripleAssPrintStream);
		
		int sum = 0; 
		int numSCUs = 0;
		for (int i=0; i<assignment.length; i++)
		{
			sum = sum + costs[assignment[i][0]][assignment[i][1]];
			if (costs[assignment[i][0]][assignment[i][1]] != 0) {
			    System.out.println(h.get(assignment[i][0]) + "\n" + assignment[i][1] + "\n");
				numSCUs++;
			}
	...
	private static int hg_step1(int step, int[][] cost)
	{
		//What STEP 1 does:
	...
	private static int hg_step2(int step, int[][] cost, int[][] mask, int[]rowCover, int[] colCover)
	{
		//What STEP 2 does:
	...
	private static int hg_step4(int step, int[][] cost, int[][] mask, int[] rowCover, int[] colCover, int[] zero_RC)
	{
		//What STEP 4 does:
	...
	private static int hg_step6(int step, int[][] cost, int[] rowCover, int[] colCover, int maxCost)
	{
		//What STEP 6 does:
	...
	private static int findSmallest		//Aux 1 for hg_step6.
